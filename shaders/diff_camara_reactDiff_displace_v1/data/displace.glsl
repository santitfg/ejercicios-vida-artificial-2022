...
void main(void){
    ...
    vec4 col1=texture2D(texB,fract(uv+col0.rg*mult));// vec2(uv.x,1.-uv.y);
    // vec4 col1=mix(texture2D(texB,fract(uv+col0.rg*mult)),texture2D(texB,uv),filtro);// vec2(uv.x,1.-uv.y);
