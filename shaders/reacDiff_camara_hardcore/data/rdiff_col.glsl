...
void main(void){
  ...
  if(click){
    ...
    gl_FragColor=clamp(sum+texture2D(ppixels,st),vec4(0.),vec4(1.));
    ...
    float componentA=getR(ppixels,0.,.0);//component A
    float componentB=getG(ppixels,0.,.0);//component B
    ...
    laplacianA+=getR(ppixels,1.,.0)*adj;
    laplacianA+=getR(ppixels,.0,1.)*adj;
    laplacianA+=getR(ppixels,-1.,.0)*adj;
    laplacianA+=getR(ppixels,.0,-1.)*adj;
    
    laplacianA+=getR(ppixels,1.,1.)*diag;
    laplacianA+=getR(ppixels,-1.,-1.)*diag;
    laplacianA+=getR(ppixels,-1.,1.)*diag;
    laplacianA+=getR(ppixels,1.,-1.)*diag;
    ...
    */
    ...
    laplacianB+=getG(ppixels,1.,.0)*adj;
    laplacianB+=getG(ppixels,.0,1.)*adj;
    laplacianB+=getG(ppixels,-1.,.0)*adj;
    laplacianB+=getG(ppixels,.0,-1.)*adj;
    
    laplacianB+=getG(ppixels,1.,1.)*diag;
    laplacianB+=getG(ppixels,-1.,-1.)*diag;
    laplacianB+=getG(ppixels,-1.,1.)*diag;
    laplacianB+=getG(ppixels,1.,-1.)*diag;
