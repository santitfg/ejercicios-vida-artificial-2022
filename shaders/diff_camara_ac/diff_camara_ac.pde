...
PGraphics bufferDiff, frameAnterior, sumDiff, bufferAc, bufferMult, buffer;
PShader shaderDiff, shaderSum, ac, shaderMult, shaderSumThresh; //prefiero procesar las imgs en GPU 
...
void setup() {
  ...
  shaderMult=loadShader("mult.glsl");  
  ...
  bufferMult=createGraphics(cam.width, cam.height, P2D);
...
void draw() {
  ...
  /*
  shaderMult.set("texture1", cam);
   bufferMult.beginDraw();
   bufferMult.image(sumDiff, 0, 0, cam.width, cam.height);
   
   bufferMult.filter(shaderMult);
   bufferMult.endDraw();
   */
  ...
  image(bufferAc, 0, 0, width, height);
